...
function edit_result(){
 ...
 $updateArray = array();
        for($x = 0; $x < sizeof($id); $x++){
			...
			if(empty($is_absent[$x])){$absent=0;}else{$absent=1;}
			//$tot[]=$total_marks;
            $updateArray[] = array(
                'id'=>$id[$x],
                'grp_a_marks' => $grp_a_marks[$x],
   ...
   //print_r($admit_id);print_r($subject_id);exit;
	for($x = 0; $x < sizeof($subject_id); $x++){
	  ...
	  $data[] = array(
		...
		$query=$this->db->query("select id from student_admit where subject_id='".$subject_id[$x]."' and student_id='".$id."'")->result_array();
		$query[0]['id'];
		//if($admit_id[$x]=)
		$aid=$admit_id[$x];	
