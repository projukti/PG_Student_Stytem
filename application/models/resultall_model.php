<?php
error_reporting(0);
Class resultall_model extends CI_Model
{
...
$condition = "result.student_id ='".$id."' and student_sem.student_id ='".$id."'";
$this->db->select('result.*,student_sem.roll,student_sem.no,session.session_yr,result.sem');
$this->db->from('result');
$this->db->join('student_sem', 'result.sem = student_sem.sem');
$this->db->join('session', 'student_sem.session = session.id');
$this->db->where($condition);
$query = $this->db->get();
if ($query->num_rows() > 0) {
foreach ($query->result() as $row) {
