...
$this->db->select('student_admit.*,subjects.paper_name','student_admit.session');
$this->db->from('student_admit');
...
function insert_result(){
 ...
 ///admit session
 $session_admit = $this->input->post('session_admit');
 ...
 $i=0;
	  ...
	  $data = array(
		'student_id'=> $this->input->post('id'),
		'sem' => $this->input->post('sem'),
		'session'=> $this->input->post('session_admit'),
		'total'=>array_sum($tot),
...
$count_nc_paper=$this->db->query("select id from student_admit where result_status='NC' and disposed!='Yes' and student_id='".$student_id."' and sem='".$sem."'")->num_rows();
	  ...
	  $condition = "student_id ='".$student_id."' AND sem ='".$sem."' and session='".$session_admit."'";	 
	  $this->db->where($condition);
...
//	  else if($current_sem=='4th'){$rolln="48".date('y', strtotime('+1 year'));}
	  ...
	  ///////////////////////////////insert and upadete semetser/////////////////////////
       ...
       if ($query4->num_rows() > 0) {
         ...
         $count_result_final=$this->db->query("select id from result_final where student_id='".$this->input->post('id')."'")->num_rows();
	  ...
	  $this->db->where($condition);
	  $this->db->update('result_final',$datafinal);	
				}
...
$count_nc_prev_sem=$this->db->query("select id from student_admit where result_status='NC' and disposed!='Yes' and student_id='".$student_id."' and sem='".$sem."'")->num_rows();	  
	  if(($percentage>=40)&&($count_nc==0)&&($count_nc_prev_sem==0)){$status='Q';}else{$status='NQ';}
		...
		$condition = "sem ='".$sem."' and session='".$session_admit."' and student_id='".$student_id."'";	 	 
	    $this->db->where($condition);
...
function insert_review(){
   ...
   $ss = $this->input->post('session_admit');
   $subject_id = $this->input->post('subject_id');
