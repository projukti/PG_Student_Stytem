...
function student_subject($id,$sm){
 ...
 $status = $this->db->select('status')->order_by('id', 'ASC')
                  ->get_where('student_sem', array('student_id' => $id,'sem' => $sm))
                  ->row()
                  ->status;
$restatus = $this->db->select('status')->order_by('id', 'DESC')
                  ->get_where('student_sem', array('student_id' => $id,'sem' => $sm))
                  ->row()
                  ->status;				  
$readmit = $this->db->select('readmit')->order_by('id', 'DESC')
                  ->get_where('student_sem', array('student_id' => $id,'sem' => $sm))
                  ->row()
                  ->readmit;
if($restatus=='R' && $readmit == 1)	
{
$condition = "student_sem.student_id ='".$id."' and student_sem.sem ='".$sm."' and student_sem.status ='R'";	
...
$condition = "student_admit.student_id ='".$id."' and student_sem.student_id ='".$id."' and student_sem.sem ='".$sem."' and  student_admit.sem='".$sem."' and student_admit.session='".$session."' and student_sem.status='R'";
