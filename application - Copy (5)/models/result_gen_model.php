...
$condition = "result_status ='NC' and disposed!='Yes' and student_id='".$student_id."'";
$this->db->select('id');
$this->db->from('student_admit');
$this->db->where($condition);
$query = $this->db->get();
...
$count_nc_prev=$this->db->query("select id from student_admit where result_status='NC' and disposed!='Yes' and student_id='".$student_id."'")->num_rows();
	  ...
	  if($status=='Q'){if(($count_nc==0)&&($count_nc_prev==0)){$status_sem='Q';$admit=2;}else{$status_sem='NQ';$admit=0;}}else{$status_sem='NQ';$admit=0;}
